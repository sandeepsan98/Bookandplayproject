...
function playerData(array = []) {
  ...
  array.forEach((item) => {
    ...
    anchor.setAttribute(
      "href",
      `../../pages/player/playerprofile.html?player_id=${item.requested_userDetails.user_id}`
    );
...
declinebtn.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    const reqIndex = requestrecords.findIndex(
      (request) => request.requested_user == req_userId
    );
